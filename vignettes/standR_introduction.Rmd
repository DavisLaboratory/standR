...
# Introduction
The `standR` package is short for **S**patial **t**ranscriptomics **a**nalyses a**n**d **d**ecoding in **R**, it aims at providing good practice pipeline and useful functions for users to analyse Nanostring's GeoMx DSP data, including data with GeoMX protein (~30-60 markers), CTA (cancer transcriptome atlas, ~1800 genes) and WTA (whole transcriptome atlas, ~18000 genes) panels.

The data analysis workflow consists of the following steps:

```{r echo=FALSE, fig.width=10}
image <- imager::load.image("../man/figures/workflow.jpg")
...
The purpose of this vignette is to demonstrate how to use standR to analyse Nanostring GeoMx DSP data.
...
download.file(paste0(url_base,"Kidney_Raw_TargetCountMatrix.txt"), destfile = paste0(dirPath,"/Kidney_Raw_TargetCountMatrix.txt"))
download.file(paste0(url_base,"Kidney_Sample_Annotations.txt"), destfile = paste0(dirPath,"/Kidney_Sample_Annotations.txt"))
download.file(paste0(url_base,"Kidney_Feature_Annotations.txt"), destfile = paste0(dirPath,"/Kidney_Feature_Annotations.txt"))
```
...
spe <- readGeoMx(dirPath, countFile, sampleAnnoFile, featureAnnoFile = featureAnnoFile, hasNegProbe = T)
...
colData(spe)$regions <- paste0(colData(spe)$region,"_",colData(spe)$SegmentLabel) %>%
  str_replace("_Geometric Segment","") %>%
...
```{r}
standR::plotPCA(spe, assay = 2, col = SlideName, shape = regions)
...
Here we plot the UMAP of our data. Similar variation can be observed like PCA and MDS aboved.
...
plotDR(spe, dimred = "UMAP", col = disease_status)
```


### Other PCA-related visualisations
...
plotPairPCA(spe, n_dimension = 4, col = regions, 
...
As we observed the technical variations in the data in both RLE and PCA plots. It is necessary to perform normalisation in the data.

In the `standR` package, we offer normalisation options including TMM, RPKM, TPM, CPM, upperquartile and sizefactor. Among them, RPKM and TPM required gene length information (add `genelength` column to the `rowData` of the object). For TMM, upperquartile and sizefactor, their normalised factor will be stored their `metadata`.

Here we used TMM to normalise the data.
...
Then we use RLE and PCA plot to assess the normalisation count.
...
```{r fig.width=12, fig.height=10}
plotPairPCA(spe_tmm, assay = 2, n_dimension = 4, color = SlideName)
plotPairPCA(spe_tmm, assay = 2, n_dimension = 4, color = disease_status)
plotPairPCA(spe_tmm, assay = 2, n_dimension = 4, color = regions)
```
...
In the `standR` package, we provide three approaches of removing batch effects, including RUV4, combat and limma.
...
Here we can find the best k by using function `findBestK` to examine the silhouette score of ruv4-normalised count data clustering according to batch from k = 1 to 10, where the score is closing to zero means we are more likely to successfully remove the batch effect from the data. In this case, k = 5 is the satisfied k.
...
plotPairPCA(spe_ruv, assay = 2, color = disease_status, title = "RUV4")

plotPairPCA(spe_ruv, assay = 2, color = regions, title = "RUV4")
...
```{r fig.width=12, fig.height=10}
plotPairPCA(spe_lrb, assay = 2, color = disease_status, title = "Limma removeBatch")

plotPairPCA(spe_lrb, assay = 2, color = regions, title = "Limma removeBatch")
```
...
Besides looking at the PCA plots of normalised count, another way to evaluate the batch correction is by summarising statistics such as adjusted rand index, jaccard similarity coefficient, silhouette coefficient and etc. We can do this by using the `plotClusterEvalStats` function. In the biology section, higher the score is better while in the batch section, lower is better.

We can see that when it comes to stratifying for biology factors (disease status and tissue regions), the comBat-correted method has the highest score in ARI and Jaccard (both are measuring the similarity of clusters), while in the silhouette score, which is measuring the disease between each dot to the cluster, RUV4 outperform others in displaying biological variations. On the other hand, when it comes to measuring batch clustering, RUV4 outperform others in all three statistics for this dataset.
...
Moreover, we can also have a look at the RLE plots of the normalised count to determine which batch correction performs better for this dataset.
